...
export function Dashboard() {
  ...
  useEffect(() => {
    ...
    ]).then(([nextSnapshot, nextStrategy, nextLaps, nextTyres, nextTraffic, nextTrace, nextEvaluations, nextCapabilities]) => {
  ...
  useEffect(() => {
    ...
    }, lap >= totalLaps ? 0 : 1_500);
    return () => window.clearTimeout(timer);
  }, [lap, playing, totalLaps]);
  ...
  const refreshing = Boolean(session && snapshot?.cutoff_lap !== lap);
  ...
  return (
    <main className="shell">
      <header className="topbar">
        ...
        <div className={`sync ${refreshing ? "refreshing" : ""}`}>{refreshing ? <RefreshCw /> : <span />}<div>{refreshing ? "UPDATING" : "DATA SYNCED"}<small>cutoff-safe replay</small></div></div>
      </header>
